...
export default function Fallback({ error, resetErrorBoundary }) {
    ...
    const errMap: ErrDiagnosisMap = {
      lookmlNotFoundErr: {
        diagnosis: "The error is likely related to your LookML `.env` variables.",
        steps: [
      ...
      lookmlExamplesUndefinedErr: {
        ...
        steps: [
          ...
          "If not, please upload examples for that explore id by following the README instructions.",
      ...
      bigQueryExamplesErr: {
        ...
        steps: [
          ...
          "Ensure that examples exist in both the Examples and Refinement tables."
        ]
      },
      bigQueryModelErr: {
        ...
        steps: [
          "Please read the error message from BigQuery, the SQLException error will give you the reason.",
          "Ensure that your BQ service account used for the VERTEX_BIGQUERY_LOOKER_CONNECTION_NAME env variable has BigQuery Connections User permission to the remote connection provisioned by Terraform",
    ...
    useEffect(() => {
      if(error.message) {
        if(error.message.includes("Java::NetStarschemaClouddbJdbc::BQSQLException:")){
          ...
          setErrorMessage(errMap.lookmlModelNotFoundErr)
        } 
            ...
            <div className={styles.errorCard}>
              ...
              <div className={styles.errorContent}>
                <div className={styles.errorSection}>
                  <h3>Error Name:</h3><p>{errorMessage ? error.name: ''}</p>
                </div>
                <div className={styles.errorSection}>
                  <h3>Error Message:</h3><p className={styles.messageContainer}>{errorMessage ? error.message: ''}</p>
                </div>
                <div className={styles.errorSection}>
                  <h3>Diagnosis:</h3>
                  <p>{errorMessage ? errorMessage.diagnosis :''}</p>
                </div>
                <div className={styles.errorSection}>
                  <Accordion2 label="Troubleshooting Steps" indicatorPosition="left" density={1}>
                  <ul>
                    {errorMessage && errorMessage.steps ? 
