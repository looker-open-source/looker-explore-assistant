...
import { LookerEmbedExplore } from '@looker/embed-sdk'
...
const AppInternal = () => {
  ...
  const initialize = async () => {
    ...
    setData(responses)
    let { fields } = await core40SDK.ok(core40SDK.lookml_model_explore(
      LOOKER_MODEL, LOOKER_EXPLORE, 'fields'))
      // 'thelook', 'order_items', 'fields'))
      // 'sfdc_demo', 'opportunity_line_item', 'fields'))
    let dimensions = fields.dimensions.map((field: any) => {
      const {name, type, description} = field
      return 'name: ' + name + ', type: ' + type + ', description: ' + description + '\n'
    })
    let measures = fields.measures.map((field: any) => {
      const {name, type, description} = field
      return 'name: ' + name + ', type: ' + type + ', description: ' + description + '\n'
    })
          ...
          <SpaceVertical>
            ...
            <div style={{
              ...
              <FieldTextArea
                ...
                width={"100%"}//window.innerWidth > 768 ? '70%' : 'auto'}
                />
              <div style={{marginTop:'1rem', marginBottom:'1rem',display:'flex', flexDirection:'row', width:'100%',height:'100%', justifyContent:'space-between',alignContent:'center',alignItems:'center'}}>
                <div style={{width:'50%'}}>
                <Button disabled={submit} onClick={() => handleSubmit(undefined)} style={{width:'100%'}}>{submit ? <BardLogo search={true}/> : 'Run Prompt'}</Button>
                </div>
              </div>
              <Tabs2 distributed>
                  <Tab2 id="examples" label="Example Prompts">
                  <div className={styles.scrollbar} style={{overflowY:'scroll',height:'40vh'}}>
                  {categorizedPrompts.map((item) => 
                  <div className={styles.card} onClick={() => {
                    handleExampleSubmit(item.prompt)
                  }}>
                    <span style={{color:`${item.color}`,fontSize:'1.3vh'}}>{item.category}</span>
                    <span style={{fontSize:'2vh'}} id="examplePrompt">{item.prompt}</span>
                  </div>
                  )}
                  </div>
                  </Tab2>
                  <Tab2 id="history" label="History">
                    <div className={styles.scrollbar} id="historyScroll" style={{overflowY:'scroll',height:'40vh'}}>
                    {db && data.length > 0 && data.filter(item => item.message !== '').map((item,index) => {
                        return (
                            <div onClick={(e) => handleExampleSubmit(item.message)} className={styles.card}>
            ...
            {exploreUrl && 
              ...
              {exploreUrl && <ExploreEmbed exploreUrl={exploreUrl} setExplore={setExplore} setSubmit={setSubmit}/>}
              </div>
              }
            </div>
            </div>
...
const LandingPage = ({begin}) => {
  const docs = [
    {
      title:'No Code Prompt Tuning',
      model:'Vertex AI Generative AI Studio',
      description:'No code prompt tuning of foundational model with generated Python code for engineer hand off.',
      doc:'https://cloud.google.com/vertex-ai/docs/generative-ai/learn/generative-ai-studio'
    },
    {
      title:'Generate Text',
      model:'text-bison-001',
      description:'Generative Text Model by Google. Used to Generate explore expanded url parameters. This is done based off 20 examples of question answer that is fed into the prompt context.',
            ...
            return (
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <img height="70%" width="70%" src={index === 0 ? "https://lh3.googleusercontent.com/-1brN-k2sapOWO4gfdJKGEH8kZbfFjrzEMjNs1dl4u64PBH-yxVmB5vG2aHDatRudSByL3lwViUg1w" : "https://developers.generativeai.google/static/site-assets/images/marketing/home/icon-palm.webp"}/>
                </div>
                <div style={{
                  paddingTop:'1rem',
                  paddingLeft:'1rem',
