...
This is not necessary for a POC, but is necessary for a production deployment. To ensure access to the cloud function is secure, you can restrict network access to a Google Cloud Run service or function to only allow requests from your Looker server by using a combination of Google Cloud Armor and a serverless VPC connector. Here are the steps to achieve this:

First, determine the host IP range for your Looker instance. You can choose arbitrary names for the VPC_NETWORK_NAME, CONNECTOR_NAME, SUBNET_NAME

Use the Cloudshell link to open a [GCP Shell](https://ssh.cloud.google.com/cloudshell/editor?cloudshell_git_repo=https://github.com/bytecodeio/looker-explore-assistant&cloudshell_tutorial=./explore-assistant-backend/security_README.md&shellonly=true&cloudshell_git_branch=marketplace_deploy)

Create a VPC Network: If you don't already have a VPC network, create one.
```bash
...
Create a Serverless VPC Connector: This will allow your Cloud Run service to use the VPC network.
```bash
gcloud compute networks vpc-access connectors create [CONNECTOR_NAME] \
    ...
    --range [IP_RANGE]
```
Deploy your Cloud Run Service with the VPC Connector:
```bash
...
Create a Google Cloud Armor Security Policy: This policy will restrict access based on IP addresses.
```bash
gcloud compute security-policies create [POLICY_NAME] \
    --description "Restrict access to specific IP addresses"
```

Add Rules to the Security Policy:
```bash
...
By following these steps, you can restrict network access to your Google Cloud Run service or function to only allow requests from specific IP addresses while keeping the requests unauthenticated.
